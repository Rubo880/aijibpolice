...
import { getSources, upsertOpportunity, listNewOpportunities, listNewOpportunitiesBySourceKind, markOpportunityStatus } from './db.js';
...
export async function scanHHNow() {
  ...
  const started = Date.now();
  const { items, errors, source: discoverySource = 'unknown' } = await searchHH();

  if (!items.length && errors.length) {
    const details = errors.slice(0, 3).map((x) => `${x.query}: ${x.error}`).join(' | ');
    throw new Error(`HH source failed: ${details}`);
  }
...
export async function pushNewNow(chatId, limit = 8, sourceKind = null) {
  const min = Number(process.env.MIN_MATCH_SCORE || 65);
  const fetchLimit = Math.max(limit * 3, limit);
  const rows = sourceKind
    ? await listNewOpportunitiesBySourceKind(sourceKind, fetchLimit)
    : await listNewOpportunities(fetchLimit);

  let sent = 0;

  for (const o of rows) {
    const rescored = scoreOpportunity(o.raw_text || '', o.title || '');
    if (rescored.score < min) {
      await markOpportunityStatus(o.id, 'skipped');
      continue;
    }

    o.match_score = rescored.score;
    o.match_reason = rescored.reason;
