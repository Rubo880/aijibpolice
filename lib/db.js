...
export async function getHHSettings() {
  ...
  return {
    oauth: settings.hh_oauth || null,
    resumeId: settings.hh_resume_id || null
  };
}

export async function saveHHAuth({ accessToken, refreshToken, expiresIn, tokenType = 'bearer' }) {
  const sql = db();
...
export async function setHHResumeId(resumeId) {
  const sql = db();
  const rows = await sql`
    UPDATE profile
    SET settings = jsonb_set(
...
export async function clearHHAuth() {
  const sql = db();
  const rows = await sql`
    UPDATE profile
    SET settings = (COALESCE(settings, '{}'::jsonb) - 'hh_oauth' - 'hh_resume_id'),
        updated_at = NOW()
    WHERE id = 1
    RETURNING *
  `;
